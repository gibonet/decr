...
In short, `boot_nopodec_mean` performs a nonparametric bootstrap, stratified by the strata defined by the variables used for each group, and returns an object of class `boot` (from package `boot`). Here below an example with `R = 500` bootstrap replicates:
