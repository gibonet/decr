...
Note that in the quantile case, unlike in the average/mean case, the decomposition is done only in the common support. The example above decomposes the difference between the median wage of men and women in the common support in two components. The decomposition can be done for any other quantile level (between 0 and 1), by using the argument `probs` in the function `dec_quantile`. For example, for the 25th percentile:
