...
# }
unite2_ <- function(data, col, from, remove = FALSE){
  col <- rlang::sym(col)
  dots <- rlang::syms(from)
  tidyr::unite(data = data, col = !! col, !!! dots, remove = remove)
}
# common_support2
...
spread2_ <- function(data, key, value){
  ...
  tidyr::spread(data = data, key = !! key, value = !! value)
}
# common_support2
